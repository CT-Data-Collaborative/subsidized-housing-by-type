...
for (i in 1:length(raw_data)) {
  ...
  govt <- fileCols[grepl("Government", fileCols, ignore.case = T)]
  chfa <- fileCols[grepl("CHFA", fileCols, ignore.case = T)]
  deed <- fileCols[grepl("restricted", fileCols, ignore.case = T)]
...
write.table(
  ...
  file.path(getwd(), "data", "subsidized-housing-by-type_2016.csv"),
  sep = ",",
  row.names = F
)
